...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	 ...
	 * Input Clock is 8MHz
	 *
	 * PWM frequency is equal to (input clock) / ((prescaler + 1)*(ARR + 1))
	 *
	 * The ARR and the prescaler multiply together to create the desired frequency
	 *
	 * In this example, a prescaler of 8000-1 sets the frequency to 1000Hz
	 * The ARR of 4-1 futher reduces the frequency to 250Hz
	 *
	 * MODIFY FREQUENCY VALUES IN MX_TIM2_INIT()
	 *
	 * TO set DUTY CYCLE, change the CCR. The duty cycle = CCR/ARR.
	 * As the ARR is currently 4, the CCR is set to 2
	 *
	 * Start PWM
	 *
	 * MODIFY THE LINES BELOW TO CHANGE CCR AND START
	 */



	__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, 2); //CHANGE CCR HERE - THIRD ARGUMENT
  ...
  /* USER CODE BEGIN WHILE */
	while (1) {
		...
		//SET RDSEL - 00, 01, 10, 11

		WRITE_DRVCONF |= (0b00 << 4);


		//APPLY MESSAGE TO READ
		uint8_t SPI_msg24[3];
		SPI_msg24[2] = (WRITE_DRVCRTL >> 16) & 0x0F;
		SPI_msg24[1] = (WRITE_DRVCRTL >> 8) & 0xFF;
		SPI_msg24[0] = WRITE_DRVCRTL & 0xFF;
		...
		HAL_StatusTypeDef transmitStatus_encoder = HAL_SPI_TransmitReceive(&hspi2, SPI_msg, SPI_read, 2, 1000);

		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_12, GPIO_PIN_SET);


		//DISREGARD - TO BE VERIFIED
//		//STEPPER DRIVER WRITE AND READ
