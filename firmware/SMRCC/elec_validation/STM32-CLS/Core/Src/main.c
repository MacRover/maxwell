...
int main(void)
{
  ...
  // set channel
	...
	while (HAL_I2C_Master_Transmit(&hi2c2, pca, msg, 2, 10000) != HAL_OK)
	{
		if (HAL_I2C_GetError(&hi2c2) != HAL_I2C_ERROR_AF)
		{
			Error_Handler();
	...
	uint8_t cmd[] = {0x01, 0x10}; // Sets Pin 4 to high
	while (HAL_I2C_Master_Transmit(&hi2c2, pca, cmd, 2, 10000) != HAL_OK)
	{
		if (HAL_I2C_GetError(&hi2c2) != HAL_I2C_ERROR_AF)
		{
			Error_Handler();
